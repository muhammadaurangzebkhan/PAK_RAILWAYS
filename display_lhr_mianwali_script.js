document.addEventListener('DOMContentLoaded', () => {
    const departuresTableBody = document.getElementById('lhr-mianwali-departures-body');
    const arrivalsTableBody = document.getElementById('mianwali-lhr-arrivals-body');
    const LOCAL_STORAGE_KEY = 'trainAppSchedules';

    const renderSchedules = () => {
        const allSchedules = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || {};

        const departuresData = allSchedules.lhr_mianwali_departure || [];
        const arrivalsData = allSchedules.mianwali_lhr_arrival || [];

        populateTable(departuresTableBody, departuresData, 'lhr_mianwali_departure');
        populateTable(arrivalsTableBody, arrivalsData, 'mianwali_lhr_arrival');
    };
