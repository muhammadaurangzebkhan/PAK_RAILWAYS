document.addEventListener('DOMContentLoaded', () => {
    const departuresTableBody = document.getElementById('lhr-qta-departures-body');
    const arrivalsTableBody = document.getElementById('qta-lhr-arrivals-body');
    const LOCAL_STORAGE_KEY = 'trainAppSchedules';

    const renderSchedules = () => {
        const allSchedules = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || {};

        const departuresData = allSchedules.lhr_qta_departure || [];
        const arrivalsData = allSchedules.qta_lhr_arrival || [];

        populateTable(departuresTableBody, departuresData, 'lhr_qta_departure');
        populateTable(arrivalsTableBody, arrivalsData, 'qta_lhr_arrival');
    };
