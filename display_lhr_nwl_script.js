document.addEventListener('DOMContentLoaded', () => {
    const departuresTableBody = document.getElementById('lhr-nwl-departures-body');
    const arrivalsTableBody = document.getElementById('nwl-lhr-arrivals-body');
    const LOCAL_STORAGE_KEY = 'trainAppSchedules';

    const renderSchedules = () => {
        const allSchedules = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || {};

        const departuresData = allSchedules.lhr_nwl_departure || [];
        const arrivalsData = allSchedules.nwl_lhr_arrival || [];

        populateTable(departuresTableBody, departuresData, 'lhr_nwl_departure');
        populateTable(arrivalsTableBody, arrivalsData, 'nwl_lhr_arrival');
    };
